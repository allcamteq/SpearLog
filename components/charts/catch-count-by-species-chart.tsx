...
import type { SpeciesStat } from "@/db/queries/stats";
import "./chart-theme.css";

function ChartTooltip({ active, payload }: { active?: boolean; payload?: { payload: SpeciesStat }[] }) {
  ...
  return (
    <div className="rounded-md border border-surface-border bg-surface px-3 py-2 text-xs shadow-sm">
      <div className="font-medium">{stat.species}</div>
      <div className="text-muted">{stat.totalQuantity} caught</div>
    </div>
  );
}

export function CatchCountBySpeciesChart({ data }: { data: SpeciesStat[] }) {
  if (data.length === 0) {
